...
namespace IdentityVerification.Api.Controllers
{
    ...
    public class UserResponsesController : ControllerBase
    {
        ...
        public UserResponsesController(IUserResponseService service) => _service = service;
        ...
        [HttpPost]
        [Authorize]
        public async Task<IActionResult> Create([FromBody] CreateUserResponseDto dto, CancellationToken ct)
        {
            var created = await _service.CreateAsync(dto, ct);
            return CreatedAtAction(nameof(Get), new { id = created.ResponseID }, created);
        }

        [HttpPut("{id:int}")]
        [Authorize]
        public async Task<IActionResult> Update(int id, [FromBody] UpdateUserResponseDto dto, CancellationToken ct)
        {
